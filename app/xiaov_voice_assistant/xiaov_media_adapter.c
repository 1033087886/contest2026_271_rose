...
#include <errno.h>
#ifndef CONFIG_EXAMPLES_XIAOV_AUDIO_ALSA
...
#ifndef CONFIG_EXAMPLES_XIAOV_AUDIO_ALSA
static void xv_player_event(void *cookie, int event, int result,
                            const char *extra) {
  xv_media_adapter_t *adapter = cookie;
  (void)extra;
  if (event == MEDIA_EVENT_COMPLETED) {
    pthread_mutex_lock(&adapter->lock);
    adapter->playback_completed = result >= 0;
...
static int xv_capture_open(xv_media_adapter_t *adapter) {
  ...
  adapter->recorder = adapter->alsa.capture_handle;
#else
  void *recorder;
  ...
  ret = media_recorder_prepare(recorder, NULL, XV_MEDIA_OPTIONS);
  if (ret < 0) {
    (void)media_recorder_close(recorder);
    return ret;
  }
...
static void xv_capture_close(xv_media_adapter_t *adapter, bool notify) {
  ...
  if (adapter->recorder != NULL) {
#ifdef CONFIG_EXAMPLES_XIAOV_AUDIO_ALSA
    ret = xv_alsa_capture_close(&adapter->alsa);
    if (ret < 0) {
      result = ret;
    }
    adapter->recorder = NULL;
#else
    ret = media_recorder_stop(adapter->recorder);
    if (ret < 0) {
      result = ret;
    }
    ret = media_recorder_close(adapter->recorder);
    if (ret < 0 && result == 0) {
      result = ret;
    }
    adapter->recorder = NULL;
...
#else
    ret = (int)media_recorder_read_data(
        adapter->recorder,
        (uint8_t *)adapter->capture_frame + adapter->capture_bytes,
        XV_MEDIA_FRAME_BYTES - adapter->capture_bytes);
    if (ret <= 0) {
      xv_capture_fail(adapter, ret < 0 ? ret : -EIO);
      continue;
    }

    if ((size_t)ret > XV_MEDIA_FRAME_BYTES - adapter->capture_bytes) {
      xv_capture_fail(adapter, -EOVERFLOW);
      continue;
    }
    adapter->capture_bytes += (size_t)ret;
    if (adapter->capture_bytes == XV_MEDIA_FRAME_BYTES) {
      xv_capture_deliver(adapter);
      adapter->capture_bytes = 0U;
    }
#endif
...
static int xv_playback_open(xv_media_adapter_t *adapter) {
  ...
  adapter->player = adapter->alsa.playback_handle;
#else
  void *player;

  player = media_player_open(MEDIA_STREAM_TTS);
  if (player == NULL) {
    return -ENODEV;
  }

  ret = media_player_set_event_callback(player, adapter, xv_player_event);
  if (ret < 0) {
    (void)media_player_close(player, 0);
    return ret;
  }

  ret = media_player_prepare(player, NULL, XV_MEDIA_OPTIONS);
  if (ret < 0) {
    (void)media_player_close(player, 0);
    return ret;
  }
...
static void xv_playback_close(xv_media_adapter_t *adapter, bool notify) {
  ...
  if (adapter->player != NULL) {
    ...
    adapter->player = NULL;
#else
    ret = media_player_stop(adapter->player);
    if (ret < 0) {
      result = ret;
    }
    ret = media_player_close(adapter->player, 0);
    if (ret < 0 && result == 0) {
      result = ret;
    }
    adapter->player = NULL;
...
static int xv_playback_write_current(xv_media_adapter_t *adapter,
  ...
  return xv_alsa_playback_write(&adapter->alsa,
                                ...
                                sample_count);
#else
  const uint8_t *data = (const uint8_t *)adapter->playback_current;
  ...
  while (remaining > 0U) {
    ...
    written = media_player_write_data(adapter->player, data, remaining);
    if (written <= 0) {
  ...
  for (;;) {
    ...
    if (sample_count == 0U) {
      if (xv_playback_should_close_input(adapter)) {
        ...
        pthread_mutex_unlock(&adapter->lock);
#else
        media_player_close_socket(adapter->player);
#endif
      }
